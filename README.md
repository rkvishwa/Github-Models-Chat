...
- The browser loads all available models into a dropdown via `/api/models`.
...
- The model list comes from `https://models.github.ai/catalog/models`.

## Notes

- Keep `.env` private. It is ignored by `.gitignore`.
- Model choices come from the GitHub model catalog, not from `.env`.
